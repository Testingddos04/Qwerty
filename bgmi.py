...
ADMIN_IDS = [1860895713]
BOT_TOKEN = "7002194013:AAHjCiT76y--WGfJLVHHqCLIl1FAbjQIbpo"
    ...
    if user_id in ADMIN_IDS:
        ...
        if len(command) == 2:
            try:
                ...
                response = f"📵 𝙀𝙍𝙍𝙊𝙍 𝘿𝙈 📵 --> @KaliaYtOwner"
        else:
            response = "📵 𝙀𝙍𝙍𝙊𝙍 𝘿𝙈 📵 --> @KaliaYtOwner"
    else:
        response = f"📵 𝙀𝙍𝙍𝙊𝙍 𝘿𝙈 📵 --> @KaliaYtOwner"
...
def start_command(message):
    chat_id = message.chat.id
    bot.send_message(chat_id, "WELCOME TO 🥇PREMIUM🥇 USER--> @KaliaYtOwner.", reply_markup=main_menu())
